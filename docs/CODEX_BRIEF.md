...
## 0.5 용어 정의 (2026-08-20 확정 — UI 문구에 엄격 적용)

- **피쳐(Feature)** = **상품(프로덕트)이 소개되는 것**. "내 제품이 피쳐되었다"의 그 피쳐. 네비 "피쳐" 메뉴는 `/products`로 연결한다. CTA·배지·랭킹에서 상품 노출을 말할 때만 "피쳐" 사용.
- **스토리(Story)** = 인터뷰·브랜드 스토리 등 **언론 기사처럼 발행되는 에디토리얼 콘텐츠**. `/stories` 경로. 화면 문구에서 이 콘텐츠를 "피쳐"라고 부르지 말 것.
- 코드 내부의 `Feature` 타입/`features` 테이블 이름은 스토리 콘텐츠를 담지만 **리네임하지 않는다** (내부 명칭 유지, 화면 용어만 구분).
- ⚠️ **`src/middleware.ts`·`src/proxy.ts` 생성 금지** — 프로덕션(Cloudflare Workers + OpenNext)이 Node 미들웨어를 미지원이라 배포가 깨진다. 경로 보호는 페이지 서버 컴포넌트의 redirect로.
- **멘토 노트 = 비공개 피드백 댓글** (2026-08-20 확정). mentor 권한 유저가 작성하고 **해당 게시물의 파운더만 열람**. 공개 상세페이지에 멘토 노트를 정적 섹션으로 넣지 말 것 — `MentorNotes` 컴포넌트(Claude 관리)가 권한별 조건부 렌더링을 처리한다. 목데이터의 `mentorNote` 필드는 UI에서 렌더하지 않는다.
