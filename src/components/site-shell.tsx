...
export function Footer({ partners }: { partners: Partner[] }) {
  const partnerLinks = <div className="partner-row">{partners.map((partner, index) => <Link href={partner.href} key={`partner-${partner.name}-${index}`} title={partner.name}><img src={partner.logoUrl} alt={partner.name} /></Link>)}</div>;
