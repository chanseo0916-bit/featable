...
interface EventRow { id: string; slug: string; name: string; host: string; starts_at: string; location: string; is_online: boolean; fee: string | null; category: string; audience: string | null; apply_url: string; cover_url: string | null; status: PublishStatus }
...
export default async function AdminEventsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  let request = supabase.from("events").select("id,slug,name,host,starts_at,location,is_online,fee,category,audience,apply_url,cover_url,status", { count: "exact" });
  ...
  return <main className="admin-main shell"><AdminPageHeader eyebrow="CURATION" title="행사 관리" description="행사를 직접 등록하고 공개 상태를 관리합니다." publicHref="/events" />
    <section className="admin-list-panel"><EventForm /><AdminListTools query={query} placeholder="행사명 검색" /><div className="admin-list-head"><h2>전체 행사 <span>{count ?? 0}</span></h2></div><div className="admin-stack-list">
      {rows.map((row) => <article key={row.id}><div><Link href={`/events/${row.slug}`}>{row.name}</Link><p>{row.host} · {formatAdminDate(row.starts_at)}</p></div><StatusBadge status={row.status} /><div className="admin-row-actions"><RowControls table="events" id={row.id} isFeatured={false} status={row.status} showFeatured={false} /><AdminEditButton table="events" id={row.id} label={row.name} initial={{ name: row.name, host: row.host, startsAt: toAdminDateTimeInput(row.starts_at), location: row.location, isOnline: row.is_online, fee: row.fee ?? "", category: row.category, audience: row.audience ?? "", applyUrl: row.apply_url, coverUrl: row.cover_url ?? "" }} /><AdminDeleteButton table="events" id={row.id} name={row.name} /></div></article>)}
