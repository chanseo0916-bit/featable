...
interface PartnerRow { id: string; name: string; logo_url: string; href: string; intro: string | null; description: string | null; field: string | null; status: PublishStatus; is_featured?: boolean | null }
...
export default async function AdminPartnersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  let request = supabase.from("partners").select("id,name,logo_url,href,intro,description,field,status,is_featured", { count: "exact" });
  ...
  const { data, count } = await request.order("is_featured", { ascending: false }).order("sort_order", { ascending: true }).order("created_at", { ascending: true }).range(from, from + ADMIN_PAGE_SIZE - 1);
  ...
  return <main className="admin-main shell"><AdminPageHeader eyebrow="NETWORK" title="파트너 관리" description="Featable과 연결된 파트너의 정보와 공개 상태를 관리합니다." publicHref="/partners" />
    <section className="admin-list-panel"><PartnerForm /><AdminListTools query={query} placeholder="파트너명 검색" /><div className="admin-list-head"><h2>전체 파트너 <span>{count ?? 0}</span></h2></div><div className="admin-stack-list">
      {rows.map((row) => <article key={row.id}>
        ...
        <div className="admin-row-actions"><RowControls table="partners" id={row.id} isFeatured={Boolean(row.is_featured)} status={row.status} /><AdminEditButton table="partners" id={row.id} label={row.name} initial={{ name: row.name, logoUrl: row.logo_url, href: row.href, intro: row.intro ?? "", description: row.description ?? "", field: row.field ?? "" }} /><AdminDeleteButton table="partners" id={row.id} name={row.name} /></div>
      </article>)}
