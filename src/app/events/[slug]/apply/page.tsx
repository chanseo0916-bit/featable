...
export default async function EventApplyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="event-apply-page">
    ...
    <section className="event-apply-layout">
      ...
      <EventRegistrationCard eventId={event.id} slug={event.slug} host={event.host} mode="internal" capacity={event.capacity} approvalMode={event.approvalMode ?? "instant"} closed={event.registrationClosed ?? false} user={user ? { name: profile?.full_name?.trim() || user.user_metadata?.full_name || "Featable 멤버", email: user.email ?? "" } : undefined} registration={registration ? { status: registration.status as "verification_pending" | "pending" | "confirmed" | "waitlisted" | "rejected" | "cancelled" } : undefined} formOnly />
    </section>
  </main>;
}
