...
export default async function MyJobsPage() {
  ...
  return <><StudioNav active="jobs" /><main className="studio-dashboard managed-community-page"><div className="shell studio-dashboard-inner">
    <header className="managed-community-heading"><div><span>ORGANIZATION RECRUITING</span><h1>내 채용 공고</h1><p>내가 관리하는 브랜드·커뮤니티·파트너의 채용 소식을 직접 등록하세요.</p></div>{organizations.length ? <Link href="/my/jobs/new">새 채용 공고 +</Link> : <Link href="/my">내 조직 확인 →</Link>}</header>
    {jobs.length ? <section className="partner-job-list">{jobs.map((job) => { const organization = jobOrganization(job); return <article key={job.id}><div><small>{organization ? `${typeLabel[organization.type]} · ${organization.name}` : "ORGANIZATION"} · {job.type}</small><h2>{job.title}</h2><p>{job.role} · {job.location}</p></div><span data-status={job.status}>{job.status === "published" ? "공개 중" : "숨김"}</span><footer><Link href={`/my/jobs/${job.id}`}>수정·관리</Link>{job.status === "published" && <Link href={`/jobs/${job.slug}`}>공개 페이지 ↗</Link>}</footer></article>; })}</section> : <section className="managed-community-empty"><span>RECRUITING</span><h2>{organizations.length ? "첫 채용 공고를 올려보세요." : "관리할 수 있는 조직이 필요해요."}</h2><p>{organizations.length ? "조직을 선택하고 직무와 지원 링크를 등록하면 채용 페이지에 바로 공개됩니다." : "브랜드 편집자, 커뮤니티 운영자 또는 파트너 소유자로 등록되거나 초대받으면 공고를 게시할 수 있습니다."}</p><Link href={organizations.length ? "/my/jobs/new" : "/my"}>{organizations.length ? "채용 공고 등록하기 →" : "내 워크스페이스 확인 →"}</Link></section>}
  </div></main></>;
}
