...
export function EventSettingsEditor({ eventId, slug, galleryUrls, registrationFields }: { eventId: string; slug: string; galleryUrls: string[]; registrationFields: RegistrationField[] }) {
  ...
  const save = () => startTransition(async () => { setMessage(""); const result = await updateEventPresentation({ eventId, slug, galleryUrls: gallery, registrationFields: fields }); setMessage(result.ok ? "행사 설정을 저장했습니다." : result.error ?? "저장하지 못했습니다."); });
