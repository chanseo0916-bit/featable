...
export function EventSettingsEditor(props: EventSettingsProps) {
  ...
  return <section className="event-settings-editor">
    <header><div><span>EVENT OPERATIONS</span><h2>행사 정보와 신청 설정</h2><p>공동 주최자도 공개 정보, 신청 폼과 참가비 설정을 함께 수정할 수 있어요.</p></div><button className="button" type="button" onClick={save} disabled={pending || uploadingCover}>{pending ? "저장 중…" : "변경사항 저장"}</button></header>
