...
export default async function EventRegistrationsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: event } = await supabase.from("events").select("id,slug,name,starts_at,capacity,approval_mode,submitted_by,gallery_urls,registration_fields,is_paid,payment_account,payment_notice").eq("slug", slug).maybeSingle();
  const admin = createAdminClient();
  const { data: cohostRows } = admin && event ? await admin.from("event_cohosts").select("id,user_id,email,role,profile:profiles(full_name)").eq("event_id", event.id) : { data: [] };
  const isCohost = (cohostRows ?? []).some((row) => row.user_id === user.id);
  ...
  return <><StudioNav active="events" /><main className="studio-dashboard event-attendees-page"><div className="shell studio-dashboard-inner">
    ...
    <EventSettingsEditor eventId={event.id} slug={event.slug} galleryUrls={(event.gallery_urls ?? []) as string[]} registrationFields={(event.registration_fields ?? []) as RegistrationField[]} isPaid={Boolean(event.is_paid)} paymentAccount={event.payment_account ?? ""} paymentNotice={event.payment_notice ?? ""} />
    {event.submitted_by === user.id && <EventCohostManager eventId={event.id} slug={event.slug} initial={(cohostRows ?? []) as { id: string; email: string; role: string; profile?: { full_name?: string | null } | null }[]} />}
  </div></main></>;
}
