...
export default async function MyEventsPage() {
  ...
  return <><DashNav active="events" /><main className="dash-page my-events-page"><div className="shell dash-shell">
    <header className="my-events-heading"><div><h1>내 행사</h1><span>신청한 행사와 내가 등록한 행사를 한곳에서 관리하세요.</span></div><Link href="/events">행사 둘러보기 →</Link></header>
    <section className="my-event-section"><header><h2>내 신청</h2></header>{registrations.length ? <div className="my-event-grid">{registrations.map((item) => item.event && <EntityCard href={`/events/${item.event.slug}`} key={item.id} layout="image" media={item.event.cover_url || placeholder(`event-${item.event.slug}`)} mediaAlt={item.event.name} ratio={1.45} mediaOverlay={<span className={`my-event-status is-${item.status}`}>{statusLabel[item.status]}</span>} metaLead={<span className="entity-card-meta-lead">{formatMonthDayKst(item.event.starts_at)}</span>} metaBadge={item.event.category ? <span>{item.event.category}</span> : null} title={item.event.name} description={`${item.event.host} · ${item.event.location}${item.user_id ? "" : " · 이메일 신청"}`} />)}</div> : <div className="my-event-empty"><strong>아직 신청한 행사가 없어요.</strong><span>관심 있는 행사에 Featable로 바로 신청해보세요.</span><Link href="/events">행사 찾기 →</Link></div>}</section>
    <section className="my-event-section"><header><h2>내가 운영하는 행사</h2></header>{operatedEvents.length ? <div className="my-event-grid">{operatedEvents.map((event) => <EntityCard href={`/my/events/${event.slug}`} key={event.id} layout="image" media={event.cover_url || placeholder(`event-${event.slug}`)} mediaAlt={event.name} ratio={1.45} mediaOverlay={<span className={`my-event-status is-${event.operatorRole === "대표 주최" ? "host" : "cohost"}`}>{event.operatorRole}</span>} metaLead={<span className="entity-card-meta-lead">{formatMonthDayKst(event.starts_at)}</span>} metaBadge={event.registration_mode === "internal" ? <span>Featable 신청자 관리</span> : <span>외부 신청</span>} title={event.name} description={`${event.operatorRole} · ${event.host}`} />)}</div> : <div className="my-event-empty"><strong>아직 운영 중인 행사가 없어요.</strong><span>행사를 등록하거나 공동 주최 초대를 수락하면 이곳에 표시됩니다.</span><Link href="/my/partner/register?type=event">행사 등록하기 →</Link></div>}</section>
  </div></main></>;
}
