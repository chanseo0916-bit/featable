...
export default async function MyEventsPage() {
  ...
  return <><DashNav active="events" /><main className="dash-page managed-community-page my-events-page"><div className="shell dash-shell">
    ...
    <section className="my-event-section">{operatedEvents.length ? <><header><h2>내가 운영하는 행사</h2></header><div className="my-event-grid">{operatedEvents.map((event) => <EntityCard href={`/my/events/${event.slug}`} key={event.id} layout="image" media={event.cover_url || placeholder(`event-${event.slug}`)} mediaAlt={event.name} ratio={1.45} mediaOverlay={<span className={`my-event-status is-${event.operatorRole === "대표 주최" ? "host" : "cohost"}`}>{event.operatorRole}</span>} metaLead={<span className="entity-card-meta-lead">{formatMonthDayKst(event.starts_at)}</span>} metaBadge={event.registration_mode === "internal" ? <span>Featable 신청자 관리</span> : <span>외부 신청</span>} title={event.name} description={`${event.operatorRole} · ${event.host}`} />)}</div></> : <div className="managed-community-empty my-event-empty"><h2>아직 운영 중인 행사가 없어요.</h2><p>직접 등록하거나 공동 주최하는 행사가 여기에 표시돼요.</p><Link href="/my/partner/register?type=event">행사 등록하기 →</Link></div>}</section>
  </div></main></>;
}
