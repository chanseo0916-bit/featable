...
export default async function MyPartnersPage() {
  ...
  return <><DashNav active="partners" /><main className="dash-page managed-community-page"><div className="shell dash-shell">
    <header className="managed-community-heading"><div><h1>내 회사</h1><p>회사 프로필, 소속 커뮤니티, 팀원과 채용 공고를 한곳에서 관리하세요.</p></div><Link href="/partners/apply">파트너 등록 문의 →</Link></header>
    {partners.length ? <section className="managed-community-grid">{partners.map(({ partner, role }) => <article key={partner.id}><div className="managed-community-logo">{partner.logo_url ? <img src={partner.logo_url} alt={`${partner.name} 로고`} /> : <span>{partner.name.slice(0, 1)}</span>}</div><div className="managed-community-copy"><small>{partner.field ? `${partner.field} · ${roleName[role]}` : roleName[role]}</small><h2>{partner.name}</h2><p>{partner.intro}</p></div><span className="managed-community-status" data-status={partner.status}>{partner.status === "published" ? "공개 중" : partner.status === "hidden" ? "숨김" : "초안"}</span><footer><Link href={`/my/partners/${partner.id}`}>워크스페이스</Link>{role !== "viewer" && <Link href="/my/jobs">채용 공고</Link>}<a href={partner.href} target="_blank" rel="noreferrer">연결 페이지 ↗</a></footer></article>)}</section> : <section className="managed-community-empty"><h2>아직 참여 중인 회사가 없어요</h2><p>등록하거나 초대받은 회사의 정보를 여기에서 관리할 수 있어요.</p><Link href="/partners/apply">파트너 등록 문의하기 →</Link></section>}
  </div></main></>;
}
