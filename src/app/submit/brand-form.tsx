...
export function BrandRegistrationForm({ initial, editBrandId }: { initial?: BrandRegistrationInput; editBrandId?: string } = {}) {
  ...
  const [form, setForm] = useState({ name: initial?.name ?? "", tagline: initial?.tagline ?? "", category: initial?.category ?? "기타", description: initial?.description ?? "", website: initial?.website ?? "", logoUrl: initial?.logoUrl ?? "" });
  ...
  async function submit() {
    ...
    const result = editBrandId ? await updateStandaloneBrand(editBrandId, form) : await createStandaloneBrand(form);
    setSaving(false);
  ...
  return <section className="simple-registration-card">
    ...
    <div className="simple-brand-form">
      ...
      <div className="simple-form-fields">
        <label><span>기업명 *</span><input autoFocus value={form.name} onChange={(event) => set({ name: event.target.value })} placeholder="예: 카라멜랩" /></label>
